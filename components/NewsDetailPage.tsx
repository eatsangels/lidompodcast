...
import EditNewsModal from "../app/(routes)/noticias/[id]/EditNewsModal";
...
const NewsDetailPage: React.FC<NewsDetailPageProps> = ({ newsId, token }) => {
  ...
  useEffect(() => {
    // Obtener visitor ID usando FingerprintJS
    const getVisitorId = async () => {
      try {
        ...
        console.error("Error obteniendo el fingerprint", e);
        setError("Error al obtener el fingerprint");
      }
    };
    ...
    const fetchNews = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setError("Error al obtener la noticia. Por favor, intenta nuevamente.");
        } else {
    ...
    const fetchComments = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setError("Error al obtener los comentarios. Por favor, intenta nuevamente.");
        } else if (data) {
          setComments(
            data.map((item: any) => ({
    ...
    if (newsId) {
      ...
      // Manejar hash en la URL para desplazar al comentario
  ...
  const handleCommentSubmit = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    if (error) {
      ...
      setError("Error al agregar el comentario. Por favor, intenta nuevamente.");
    } else if (data) {
      setComments([
        {
  ...
  const handleEditSuccess = async () => {
    handleCloseModal();
    router.refresh();
    // Re-fetch comments after successful edit
    try {
      const { data, error } = await supabase
        .from("comments")
        .select("*")
      ...
      if (error) {
        ...
        setError("Error al obtener comentarios. Por favor, intenta nuevamente.");
      } else if (data) {
        setComments(
          data.map((item: any) => ({
            id: item.id as string,
  ...
  const handleLike = async (comment: Comment) => {
    if (!visitorId) return; // Si no se obtuvo visitorId, no se permite dar like
    ...
    if (error) {
      ...
      setError("Error al agregar like. Por favor, intenta nuevamente.");
    } else {
      setComments(
        comments.map((c) =>
  ...
  const displayedContent = isExpanded ? news.content : news.content.slice(0, 300);
